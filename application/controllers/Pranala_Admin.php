...
class Pranala_Admin extends CI_Controller
{
	...
	public function tambah()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($_FILES['logo']['error'] == 4) {
				$this->session->set_flashdata('message', '<div class="alert alert-danger alert_dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Anda belum mengupload logo pranala</div>');
	...
	public function hapus($id)
	{
		...
		$gbr = $this->pranala_model->get_gambar($id); //ini array
		unlink(FCPATH . 'assets/img/pranala/' . $gbr[0]);
